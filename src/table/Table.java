...
public class Table {
    ...
    private void createTable() {
        ...
        int idPiece = 1;
        // For passando por cada casa do tabuleiro e setando o valor 0 ou -1 começando
        // em 0 e seta as posições iniciais
        // Se a linha for menor que 3 e x = 0 e se linha maior que 4 e x = 0 a casa
        // recebe idPeca (Sempre quando atribuir a casa uma peça ele acrescenta um no id
        // auxiliar)
        // Quando passa pra próxima linha ele reajusta o valor do x para 1 ou 0 para n
        // ficar igual a linha anterior
        for (int i = 0; i < 8; i++) {
            x = (i % 2 == 0) ? 1 : 0;
            for (int j = 0; j < 8; j++) {
                x = (x == 0) ? 1 : 0;
